...
const getBookingHistoryByHotelId = asyncHandler(async (req: any, res: any) => {
  ...
  try {
    const hotelServiceUrl =
      process.env.HOTEL_SERVICE_URL || "http://hotel:3001/api/v1/hotels";
    const response = await axios.get(
      `${hotelServiceUrl}/booking-history/${hotelId}`,
    );
    adminData.set(
      `booking_history_${hotelId}`,
    ...
    console.error(
      "Error fetching booking history from Hotel service:",
...
const getGuestStatus = asyncHandler(async (req: any, res: any) => {
  ...
  try {
    ...
    const response = await axios.get(
      `${bookingServiceUrl}/guest-status/${hotelId}`,
    );
    console.log(
      "Guest status response from Booking service:",
      response.data.data ?? response.data,
    );
    return apiResponse(
      res,
    ...
    console.error(
      "Error fetching guest status from Booking service:",
      error?.response?.data || error?.message || error,
    );
    return apiError(
      res,
...
const getTransactionHistoryOfHotel = asyncHandler(
  async (req: any, res: any) => {
    ...
    const cachedData = await adminData.get(`transaction_history_${hotelId}`);
    ...
    if (cachedData) {
      return apiResponse(
        ...
        200,
        true,
        "Transaction history retrieved successfully (from cache)",
        JSON.parse(cachedData),
      );
    }
    ...
    try {
      const response = await axios.get(
        `${bookingServiceUrl}/transaction-history/${hotelId}`,
      );
      ...
      console.error(
        "Error fetching transaction history from Booking service:",
        error?.response?.data || error?.message || error,
      );
...
const getAllReviewsForHotel = asyncHandler(async (req: any, res: any) => {
  const { hotelId } = req.params;
  if (!hotelId) {
    return apiError(res, 400, "Hotel ID is required");
  }
  try {
    const hotelServiceUrl =
      process.env.HOTEL_SERVICE_URL || "http://hotel:3001/api/v1/hotels";
    const response = await axios.get(`${hotelServiceUrl}/ratings/${hotelId}`);
    return apiResponse(
      res,
      200,
      true,
    ...
    console.error(
      "Error fetching reviews from Hotel service:",
      error?.response?.data || error?.message || error,
    );
    return apiError(
      res,
      error?.response?.status || 500,
      error?.response?.data?.message || "Unable to retrieve reviews",
      error?.response?.data?.error || error?.message,
    );
  }
});
